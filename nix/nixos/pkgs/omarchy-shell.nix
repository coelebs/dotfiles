...
let
  ...
  runtimePath = lib.makeBinPath [ bash coreutils findutils gnugrep gnused gawk jq util-linux procps systemd quickshell qt6Packages.qtimageformats hyprland uwsm inotify-tools fontconfig perl wl-clipboard ttfx ];
...
stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    ...
    cp -R bin config default shell themes applications "$out/share/omarchy/"
    ...
    while IFS= read -r command; do
      ...
      makeWrapper "$out/libexec/omarchy/$command_name" "$out/share/omarchy/bin/$command_name" --set-default OMARCHY_PATH "$out/share/omarchy" --prefix PATH : ${lib.escapeShellArg runtimePath}
      ln -s "../share/omarchy/bin/$command_name" "$out/bin/$command_name"
